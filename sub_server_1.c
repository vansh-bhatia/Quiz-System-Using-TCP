...
void *client_thread_func (void* c_info)
{
    ...
    //Receive the message from the client
        ...
        if(strcmp(client_message,"Biology")==0 )
        {
            ...
            strcat(server_message," \n\nWhat is the powerhouse of cell? (Mitochondria or Nucleus ) : ");
            ...
            if (recv(client_sock, name, sizeof(name),0) < 0)
            {
                    printf("client_thread_func Receive Failed. Error!!!!!\n");
                    return 0;
            }
            
            if(strcmp(name, "Mitochondria") == 0){
                // score++;
                // strcpy(cscore, ((char)score));
                 score++;
                strcat(server_message,"\nCorrect Answer.");
                sprintf(cscore, "\nScore : %d", score);
                strcat(server_message, cscore);
                // strcat(server_message,cscore);
            }
            else{
                strcat(server_message,"\nWrong Answer!!!");
        ...
        // CHEMISTRY
            ...
            strcat(server_message,client_message);
            strcat(server_message," \n\nWhich among the following element is an essential constituent of acids? (Nitrogen or Oxygen ) : ");
            //Send the message back to client
            if (send(client_sock, server_message, strlen(server_message),0)<0)
            {
                    printf("client_thread_func Send Failed. Error!!!!!\n");
        ...
        //PHYSICS
        else if(strcmp(client_message,"Physics")==0){
            // int score = 0;
            char name[20], cscore[10];
            sprintf(port_buffer, "%d", info->port);
            strcpy(server_message,info->ip);
            ...
            strcat(server_message,client_message);
            strcat(server_message," \n\nWhich of the following is/are a vector quantity? (Momentum or Work ) : ");
            //Send the message back to client
            if (send(client_sock, server_message, strlen(server_message),0)<0)
            {
                    printf("client_thread_func Send Failed. Error!!!!!\n");
