...
void *client_thread_func (void* c_info)
{
    ...
    //Receive the message from the client
        ...
        if(strcmp(client_message,"Geometry")==0 )
        {
            ...
            strcat(server_message,client_message);
            strcat(server_message," \n\nA polygon having 8 sides is called (Octagon or Heptagon ) : ");
        ...
        // CHEMISTRY
            ...
            strcat(server_message,client_message);
            strcat(server_message," \n\nA system of three linear equations in three variables is inconsistent if their planes intersect? (True or False) : ");
            //Send the message back to client
            if (send(client_sock, server_message, strlen(server_message),0)<0)
            {
                    printf("client_thread_func Send Failed. Error!!!!!\n");
        ...
        //PHYSICS
            ...
            strcat(server_message,client_message);
            strcat(server_message," \n\nInsert the missing number 6 10 18 34? (66 or 62) : ");
            //Send the message back to client
            if (send(client_sock, server_message, strlen(server_message),0)<0)
            {
                    printf("client_thread_func Send Failed. Error!!!!!\n");
